...
// #define SWITCH_SETTING_DIRECTION

//#define ATTINY_MCU

#define SENS_BH1750
// #define SENS_VEML7700
...
#define adc_enable()  (ADCSRA |=  (1<<ADEN))
  ...
  #define SENSOR_ADDRESS VEML7700_I2CADDR_DEFAULT
#elif defined(SENS_BH1750)
...
const uint8_t rockerUpPin = BUTTON1; //up
const uint8_t rockerInPin = BUTTON2; //in
const uint8_t rockerDownPin = BUTTON3; //down
...
#ifdef DOUBLE_HEIGHT
  #define PAGE_BIT_SHIFT 2
  #define DOUBLE_BUFFER 0
  #define LINES_PER_PAGE 4
  #define LINES_MASK 0x03
  uint8_t redrawsSettingsPage = 1; 
#elif defined(LARGE_TEXT)
  #define PAGE_BIT_SHIFT 2
  //#define DOUBLE_BUFFER 0
...
#else
  #define PAGE_BIT_SHIFT 3
  #define DOUBLE_BUFFER 0
  #define LINES_PER_PAGE 8
  #define LINES_MASK 0x07
  uint8_t redrawsSettingsPage = 1;// changed for 8 lines from 2 to 1
#endif
// #define LINES_MASK LINES_PER_PAGE-1
...
const uint16_t scrollIso[ISO_N_VALUES] PROGMEM = {100,
                        200,
                        400,500,
                        800,
                        1600};
...
const uint8_t scrollShutter[SHUTTER_N_VALUES] PROGMEM = {30,60,125,175};
...
const float scrollAperture[APERTURE_N_VALUES] PROGMEM = {2.8,3.3,
                            4,4.8,
                            5.6,6.7,
                            8,9.5,
                            11,13,
...
const uint16_t constantOptions[2] PROGMEM = {12,320};
const float lightCorrectionOptions[2] PROGMEM = {12.5,1};
...
void readValuesEEPROM(){
  for (uint8_t i=1;i<settingLines+1;i++){
    v[i] = EEPROM.read(i);
  }
}
...
void sleepWaitingForInput(uint8_t pins) {
  ...
  // #ifdef __ATTINY85__
    ...
    // #ifdef EXTRA_PWR_SAVE
    //   // for (byte i = 0; i < 6; i++) {
    //   //   pinMode(i, INPUT);      // Set all ports as INPUT to save energy
    //   //   digitalWrite (i, LOW);  //
    //   // }
    //   // wdt_reset();            // Watchdog reset
    //   // wdt_enable(WDTO_500MS);    // Watchdog enable Options: 15MS, 30MS, 60MS, 120MS, 250MS, 500MS, 1S, 2S, 4S, 8S
  ...
  void watchdogSetup(void){
    ...
    wdt_reset();
    WDTCSR |= (1<<WDCE) | (1<<WDE);
    WDTCSR = (1<<WDIE) | (0<<WDE) | (1<<WDP1) | (1<<WDP0);  // 0.125s / interrupt, no system reset
    sei();
  }
...
void setupSensor(){ // Setup the veml7700 for one-shot mode
    ...
    #ifdef SENS_VEML7700
      Wire.write(VEML7700_ALS_CONFIG);                 // Access Command Register
      Wire.write((VEML_GAIN<<ALS_GAIN) | (VEML_IT<<ALS_IT) | (VEML_PERS<<ALS_PERS));            // Using one-shot mode for battery savings
    #elif defined(SENS_BH1750)
...
float getLux(){  // Get the temperature from a DS1621
    ...
    Wire.beginTransmission(SENSOR_ADDRESS);
    #ifdef SENS_VEML7700
      Wire.write(VEML7700_WHITE_DATA);                 // read temperature (for either mode)
    #elif defined(SENS_BH1750)
      Wire.write(ONE_TIME_HIGH_RES_MODE);                 // read temperature (for either mode)
    #endif
    Wire.endTransmission();          // Send 1 byte to the slave
    Wire.requestFrom(SENSOR_ADDRESS,2); // Request 1 byte from slave

    #ifdef SENS_VEML7700
      uint16_t rawLSB = Wire.read();
      uint16_t rawMSB = Wire.read();
    #elif defined(SENS_BH1750)
      uint16_t rawMSB = Wire.read();
      uint16_t rawLSB = Wire.read();
    #endif
...
void setupOLED() {
  #ifdef LARGE_TEXT
    // oled.begin();//
    oled.begin(width, 32, sizeof(tiny4koled_init_128x32br), tiny4koled_init_128x32br);//
    oled.setMultiplexRatio(31);
  #else
...
void readInputs() {
  ...
  if (rockerDown||rockerIn||rockerUp){
...
void processInputsForSetting(uint8_t m) {
  ...
  if (m == CURRENT_SETTING_LINE) {
    #ifdef SWITCH_SETTING_DIRECTION
      inc = rockerIn;
      dec = rockerIn;
    #else
      inc = rockerDown;
...
void processInputsSwitchingSettingLine() {
  ...
  if (setting_changed == CURRENT_SETTING_LINE) {
    ...
    settingsPage = (v[CURRENT_SETTING_LINE] - 1) >> PAGE_BIT_SHIFT;
...
void setRedrawFlagForMeteredValue() {
  if (lux_reading_changed) {
    ...
    if  (!(((v[CURRENT_SETTING_LINE]-1)^(SHUTTER_VALUE-1+v[DO_APERTURE_VALUE])) >> PAGE_BIT_SHIFT)) {
      r[SHUTTER_VALUE+v[DO_APERTURE_VALUE]] = DOUBLE_BUFFER + 1;// not sure if it works
    }
    if  (!(((v[CURRENT_SETTING_LINE]-1)^(LUX_VALUE-1)) >> PAGE_BIT_SHIFT)) {
...
void OutputSettingsPage() {
  ...
  uint8_t m = settingsPage * LINES_PER_PAGE + 1;
  for (uint8_t l = 0; (l < LINES_PER_PAGE) && (m <= settingLines); l++, m++) {
    #ifdef DOUBLE_HEIGHT
      oled.setCursor(0, l<<1);
    #else
      oled.setCursor(0, l);
    #endif
...
void PositionCursor(uint8_t m) {
    #ifdef DOUBLE_HEIGHT
      oled.setCursor((strlen_P((PGM_P)pgm_read_word(&(s[m].name))) + 2) * 6, ((m - 1) & LINES_MASK)<<1);
    #else
      oled.setCursor((strlen_P((PGM_P)pgm_read_word(&(s[m].name))) + 2) * 6, (m - 1) & LINES_MASK);
    #endif
}
...
void OutputCurrentSettingLine(__attribute__((unused)) uint8_t m, uint8_t value) {
  uint8_t currentLine = (value - 1) & LINES_MASK;
  for (uint8_t l = 0; l < LINES_PER_PAGE; l++) {
    #ifdef DOUBLE_HEIGHT
      oled.setCursor(120, l<<1);
    #else
...
void loop() {
  ...
  // if (rockerIn || rockerUp || rockerDown) {
  //   lastButtonPress = millis();
  // }
  if ((millis() - lastButtonPress) > SLEEP_TIMEOUT) { 
    sleepWaitingForInput((1<<rockerUpPin) | (1<<rockerDownPin) | (1<<rockerInPin));
  }
  readInputs();
  if (!rockerIn) updateLux();
  processInputs();
  processChanges();
  updateDisplay();
}
